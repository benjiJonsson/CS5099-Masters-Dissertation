/* 
// Algorithm (1.): Simple Backtracking Sudoku Generator 
// ---------------------------------------------------------
...
// (1.1) Generate Fully Populated Sudoku Board
// ---------------------------------------------------------
...
// Final Valid Board Test Before Solving For Starting Board
// -----------------------------------------------------

function finalBoardTest(board){
    /* Function iterates over every cell in baord and adds up 
    the numbers in each row, column, and 3x3 box. If they add 
    up to 45 then they pass the check. 
    */
    ...
    for (let row of rows){
        let rowCount = 0;
        for(let i = 0; i < 9; i++){
            rowCount += board[row][i];
        }
        //console.log(rowCount);
        if (rowCount !== 45){
            console.log(`row ${row} failed check`);
            return false;
        } 
    }
    ...
    for (let col of columns){
        let colCount = 0;
        for(let i = 0; i < 9; i++){
            colCount += board[i][col]
        }
        //console.log(colCount);
        if (colCount !== 45){
            console.log(`column ${col} failed check`);
            return false;
        } 
    }
        ...
        for (let row of rows ){
            for (let col of columns){
                ...
                for(let i = 0; i < 3; i++){
                    for(let j = 0; j < 3; j++){ 
                        boxCount += board[gridRow + i][gridCol + j]
                    }
                }
                //console.log(boxCount);
                if (boxCount !== 45){
                    console.log(`column ${num} failed check`);
                    return false;
                }    
            }
        }
    console.log("Final Valid Board Test Passed")
    return true;
}


// (1.2) Solve Sudoku Board
// -----------------------------------------------------
/*
*** Ensuring Symmetry ***
...
Need to minus chosen cell row & col from 8 - Easiest way I can think 
of removing diagonally opposite cells. e.g., For 2: if chosen cell 
is [2;2], must remove [6;6].
For the first 59 subtractions, 2 numbers are removed at a time. 
After that, one number at a time is removed becasue the chnaces 
of 2 numbers leaving a single solution is slimmer. 
*/  
...
if(!finalBoardTest(populatedSudokuBoard)){
    populatedSudokuBoard = generateSudoku();
}
